...
if [[ -n $SSH_CONNECTION ]]; then
    prompt="[SSH] %{$fg[$color]%}%n%{$reset_color%}@%U%{$fg[cyan]%}%m%{$reset_color%}%u %T %B%2~%b "
    RPROMPT='${vim_mode} ${vcs_info_msg_0_}'
else
    prompt="%{$fg[$color]%}%n%{$reset_color%}@%U%{$fg[yellow]%}%m%{$reset_color%}%u %T %B%2~%b "
    RPROMPT='${vim_mode} ${vcs_info_msg_0_}'
fi
...
setxkbmap -option compose:rstrg  # compose-key
